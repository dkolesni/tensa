...
> contrastive examples are alignment-only, not full SimCLR. F-023…F-030 and H-012…H-011 fixed;
...
>
> **Device placement (2026-09-18): GPU when available, CPU otherwise.** Generated models/plans
> select CUDA, then MPS, then CPU; `device cpu` / Python `device=` forces one. The Node/browser
> reference interpreter is a labeled CPU validation oracle.
